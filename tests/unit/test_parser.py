...
class ParserTest(unittest.TestCase):
    ...
    def test_feature_has_repr(self):
        """
    ...
    def test_feature_has_scenarios(self):
        """
    ...
    def test_outline_steps(self):
        ...
        # Steps that are a part of an outline have a reference back to the
        # outline
        for outline, steps in feature.scenarios[0].evaluated:
            for step in steps:
                self.assertEqual(step.outline, outline)

        feature = Feature.from_string(FEATURE1)

        # Steps that are not a part of an outline don't have the outline
        # reference
        for outline, steps in feature.scenarios[0].evaluated:
            for step in steps:
                self.assertEqual(step, outline, None)
    ...
    def test_can_parse_feature_description(self):
        ...
        self.assertEqual(expected_scenario_names, got_scenario_names)
        self.assertEqual(len(feature.scenarios[0].steps), 4)

        step1, step2, step3, step4 = feature.scenarios[0].steps

        self.assertEqual(step1.sentence,
                         'Given I have entered 3 into the calculator')
        self.assertEqual(step2.sentence,
                         'And I have entered 2 into the calculator')
        self.assertEqual(step3.sentence, 'When I press divide')
        self.assertEqual(step4.sentence,
                         'Then the result should be 1.5 on the screen')

    def test_scenarios_parsed_by_feature_has_feature(self):
        "Scenarios parsed by features has feature"

        feature = Feature.from_string(FEATURE2)

        for scenario in feature.scenarios:
            self.assertEqual(scenario.feature, feature)

    def test_feature_max_length_on_scenario(self):
        """
        ...
        feature = Feature.from_string(FEATURE1)
        self.assertEqual(feature.max_length, 76)

    def test_feature_max_length_on_feature_description(self):
        """
        The max length of a feature considering when one of the description
        lines of the feature is longer than the remaining things
        """

        feature = Feature.from_string(FEATURE2)
        self.assertEqual(feature.max_length, 47)

    def test_feature_max_length_on_feature_name(self):
        """
        ...
        self.assertEqual(feature.max_length, 78)

    def test_feature_max_length_on_step_sentence(self):
        """
        The max length of a feature considering when the some of the step
        sentences is longer than the remaining things
        """

        feature = Feature.from_string(FEATURE4)
        self.assertEqual(feature.max_length, 55)

    def test_feature_max_length_on_step_with_table(self):
        """
        The max length of a feature considering when the table of some of the
        steps is longer than the remaining things
        """

        feature = Feature.from_string(FEATURE5)
        self.assertEqual(feature.max_length, 83)

    def test_feature_max_length_on_step_with_table_keys(self):
        """
        The max length of a feature considering when the table keys of some of
        the steps are longer than the remaining things
        """

        feature = Feature.from_string(FEATURE7)
        self.assertEqual(feature.max_length, 74)

    def test_feature_max_length_on_scenario_outline(self):
        """
        The max length of a feature considering when the table of some of the
        ...
        feature = Feature.from_string(FEATURE6)
        self.assertEqual(feature.max_length, 79)

    def test_feature_max_length_on_scenario_outline_keys(self):
        """
        ...
        self.assertEqual(feature1.max_length, 68)
        self.assertEqual(feature2.max_length, 68)

    def test_description_on_long_named_feature(self):
    ...
    def test_description_on_big_sentenced_steps(self):
    ...
    def test_single_scenario_single_scenario(self):
        ...
        self.assertEqual(first_scenario.tags, (
            'many', 'other', 'basic', 'tags', 'here', ':)'
        ))

    def test_single_feature_single_tag(self):
    ...
    def test_single_scenario_many_scenarios(self):
    ...
    def test_scenarios_with_extra_whitespace(self):
        "Make sure that extra leading whitespace is ignored"
        feature = Feature.from_string(FEATURE14)

        self.assertEqual(type(feature.scenarios), tuple)
        self.assertEqual(len(feature.scenarios), 1, "It should have 1 scenario")
        self.assertEqual(feature.name, "Extra whitespace feature")

        scenario = feature.scenarios[0]
        self.assertEqual(type(scenario), Scenario)
        self.assertEqual(scenario.name, "Extra whitespace scenario")

    def test_scenarios_parsing(self):
        ...
        self.assertEqual(scenarios_and_tags, [
    ...
    def test_scenarios_with_special_characters(self):
        """
        Make sure that regex special characters in the scenario names are
        ignored
        """
    ...
    def test_background_parsing_with_mmf(self):
        ...
        assert step2.sentence == \
            'And the following clients:'
        self.assertEqual(step2.hashes, (
            {u'Name': u'John Doe'},
            {u'Name': u'Foo Bar'},
        ))

    def test_background_parsing_without_mmf(self):
        ...
        self.assertEqual(step2.table, (
            ('Name',),
            ('John Doe',),
            ('Foo Bar',),
        ))

    def test_syntax_error_for_scenarios_with_no_name(self):
        ("Trying to parse features with unnamed "
         "scenarios will cause a syntax error")
        with self.assertRaises(AloeSyntaxError) as error:
    ...
    def test_syntax_error_malformed_feature(self):
...
""")
        ...
        self.assertEqual(error.exception.msg, '\n'.join((
            "Syntax error at: None",
            "Parser errors:",
    ...
    def test_syntax_error_malformed_feature_from_file(self):
...
""")
    ...
    def test_scenario_post_email(self):
        """
        Having a scenario which the body has an email address; Then the
        following scenario should have no tags related to the email
        """
    ...
    def test_feature_first_scenario_tag_extraction(self):
        """
        A feature object should be able to find the single tag belonging to the
        first scenario
        """
    ...
    def test_feature_first_scenario_tags_extraction(self):
        """
        A feature object should be able to find the tags belonging to the
        first scenario
        """
